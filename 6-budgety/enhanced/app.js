...
var budgetCtrltroller = (function() {
  ...
  return {
    ...
    deleteItem: function(type, id) {
      ...
      index = ids.indexOf(id); // returns the index of the element in the argument

      if (index !== -1) {
        data.allItems[type].splice(index, 1); // deletes array entrys .splice(indexnumber, howManyEntrysToDelete)
      }
    },
    ...
    calculateBudget: function() {
      ...
      if (data.totals.inc > 0) {
        data.percentage = Math.round((data.totals.exp / data.totals.inc) * 100); // Expense = 100 and income 200, spent 50% = 100 / 200 = 0.5 * 100
      } else {
        data.percentage = -1;
      }
    },
...
var UIController = (function() {
  ...
  return {
    ...
    displayPercentages: function(percentages) {
      ...
      // .forEach on noteLists not supportet in IE!
...
var controller = (function(budgetCtrl, UICtrl) {
  var setupEventListerners = function() {
    ...
    document.addEventListener("keypress", function(event) {
      // if ENTER (keyCode --> 13) was pressed add Item
  ...
  var ctrlDeleteItem = function(event) {
    ...
    if (itemID) {
      // if the clicked item has an ID

      // inc-1
      splitID = itemID.split("-");
      type = splitID[0];
      ID = parseInt(splitID[1]); // parseInt --> konvert the string to an int number
  ...
  var ctrlDeleteAllItems = function() {
    ...
    // 1. Delete all items in the data structure
